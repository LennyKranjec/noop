...
| `HRZones` | `zones(age:maxHROverride:)` → `HRZoneSet` (%HRmax); `zones(maxHR:restingHR:)` → Karvonen %HRR set (the app's display zones); `observedZoneHRmax` / `learnedZoneHRmax` / `zoneRestingHR` learn their inputs; `timeInZone(_:zoneSet:)` → `TimeInZone`. |
