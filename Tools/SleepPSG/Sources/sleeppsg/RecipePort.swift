...
enum V2Recipe {
    ...
    static func stageEpochs(_ feats: [Epoch], cfg: RecipeConfig = .shipped) -> [String] {
        ...
        for f in feats {
            ...
            // Mirrors the shipped stager: the motionless clamp holds from the window's first epoch (the pre-sleep
            // unclamp was removed there; the session-level onset trim handles still-but-awake time instead).
